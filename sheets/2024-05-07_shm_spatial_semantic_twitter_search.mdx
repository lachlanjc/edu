...
Twitter knows me wildly well, and no one else in the world has my exact feed: a mix of many kinds of design, web dev, open source, pop music, young musicians, queer media, climate science, environmental policy, and so much more. My feed is wildly compelling to me, which is why I spend so much time there. I’m intrigued that social networks calculate so many factors on posts/people—which is why they’re so good at recommending content—but compress all this metadata into one number: the ranking of posts in your feed for any given session.

This project is trying to explode back out the gems of my Twitter feed into multiple dimensions. It’s an advanced semantic search tool for my Twitter likes + bookmarks, using an infinite canvas spatial UI to make it a tool for
